...
[![Unit Tests](https://img.shields.io/badge/unit_tests-297-4c9f70?logo=nodedotjs&logoColor=white)](#tests-ausf%C3%BChren)
[![Lines of Code](https://img.shields.io/badge/lines_of_code-6051-8a7bd8?logo=javascript&logoColor=000)](#tech-stack)
[![Coverage (Unit)](https://img.shields.io/badge/coverage_(unit)-95.54%25_lines-brightgreen)](#tests-ausf%C3%BChren)
...
- 🖱️ **Karte ist anklickbar** — tippe irgendwohin in Berlin, und die Card springt auf den Kiez dieses Punkts (inkl. neuer Adresse). **Mit aktivem Sektoren-Overlay folgt die Auswahl der farbigen Ebene**: Bezirke → ganzer Bezirk, Regionen → Bezirksregion, Kieze → Kiez (Umschalten hebt eine bestehende Auswahl sofort mit an)
...
- 🗺️ **Sektoren-Overlay** (4-Stufen-Button) — *aus · Bezirke (L) · Regionen (M) · Kieze (S)*, von grob nach fein. Färbt die jeweilige Ebene **nachbarschafts-bewusst** (Distanz-2-Graph-Coloring über geteilte Grenzen) → angrenzende **und** nahe Flächen bekommen weit auseinanderliegende Farbtöne und sind klar unterscheidbar. **Jede sichtbare Fläche wird beschriftet** — Anker = **visuelle Mitte** (Pole of Inaccessibility, nicht BBox-Mittelpunkt), beim Clip die Mitte des sichtbaren Flächenteils; Anti-Jitter-Hysterese mit Randabstand; **kartografische Hierarchie** (Kollisionspriorität + Größe nach Fläche); bedrängte Labels weichen per variablem Anker aus. Die **Auswahl trägt dasselbe große Uppercase-Label** wie die Overlay-Ebene (hohe Priorität, keine Doppelung). Zusätzlich benennt eine schwebende **„Aktueller-Bereich"-Plakette** mit Farbpunkt live die Fläche in der Kartenmitte. Desktop: Overlay-Toggle und Stadt-Pill haben **feste Slot-Breiten**, damit die Topbar-Badges beim Umschalten nicht springen
...
- 🎨 **Material 3 Expressive** — Feder-Physik für Sheet/Karte (`motion.js`) plus **anime.js**-Choreografie für Card/Toasts/Chips (`fx.js`); tonale Flächen, XL-Shapes, Shape-Morph beim Tippen; `prefers-reduced-motion` überall geachtet
...
```bash
npm test                                                        # 297 Unit-Tests, Nodes eingebauter Runner, null Test-Dependencies
...
Getestet wird die **abhängigkeitsfreie Pure-Logik** — Stand heute **297 Tests, 100 % Line-Coverage**
...
| `src/overlayLabels.js` | Label-Anker: visuelle Mitte (PoI), Kandidatenraster, Clip-Pick, Selection-Anchor |
| `src/fx.js` | Delight-Layer: `prefers-reduced-motion` / Coarse-Pointer-Helfer (anime.js selbst nicht gemockt) |
| `src/poiHit.js` | Zoomabhängige POI-Klick-Toleranz (0 px Stadtübersicht → 10 px ab Kiez-Zoom) |

`main.js`/`map.js` hängen an DOM + MapLibre/WebGL und sind bewusst nicht unit-getestet — testwürdige
Logik wird stattdessen in maplibre-freie Module extrahiert (`prefs.js`, `overlayLabels.js`, `poiHit.js`). Die CI
...
| Motion | **MD3-Feder-Integrator** + **anime.js v4** | Sheet/Karte = eigene Springs; Card/Toasts/Chips = `fx.js` |
